  ...
  methods: {
    ...
    saveNewBoard() {
      boardsService.addBoard(this.newBoard).then((response) => {
        ...
        if(error.response){
          this.errorMsg = "Error submitting the new board. Resonse received was " + error.response.statusText;
        } else if(error.request){
          this.errorMsg = "Error submitting the new board. The Server could not be reached";
        } else {
          this.errorMsg = "Error submitting the new board. Request could not be created :(";
        }
      })
    },
