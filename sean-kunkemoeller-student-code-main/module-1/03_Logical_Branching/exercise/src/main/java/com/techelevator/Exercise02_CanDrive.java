...
public class Exercise02_CanDrive {
    ...
    public boolean canDrive(boolean hasPermit, boolean withLicensedPassenger, int passengerAge, boolean isPassengerOurGuardian) {
        ...
        if ((hasPermit && isPassengerOurGuardian && withLicensedPassenger) && passengerAge >= 18) {
            return true;
        }
        return false;
    }
    //if passenger hasPermit, and with licensed passenger, passengerAge must be >= 21.
    //if passenger hasPermit, withLicensedPassenger and the licensed passenger is the guardian, passengerAge must be >= 18.
}
