...
public class Exercises {
	...
	public String animalGroupName(String animalName) {
		...
		Map<String, String> animalGroupName = new LinkedHashMap<String, String>();
	...
	public Map<String, Integer> robPeterToPayPaul(Map<String, Integer> peterPaul) {
		for(Map.Entry<String, Integer> tempKeyValuePair : peterPaul.entrySet()){ //entrySet will allow you to loop over a map to get the key/value pair
			System.out.println("Peter's $: " + peterPaul.get("Peter")); //doesnt need to be in the loop to return values, will only continue multiple times
			System.out.println("Paul's $: " + peterPaul.get("Paul")); //doesnt need to be in the loop to return values, will only continue multiple times
			System.out.println(tempKeyValuePair.getKey() + " : " + tempKeyValuePair.getValue());
		}
		int halfOfPeter = peterPaul.get("Peter") / 2; //use to add or remove money from peter to paul
		//if peter money > 0 && paul money < 1000 remove half and give to paul
		if(peterPaul.get("Peter") > 0 && peterPaul.get("Paul") < 1000){     //key: peter/paul returns value (integer) attached to key
	...
	public Map<String, String> beginningAndEnding(String[] words) {
		Map<String, String> firstAndLast = new HashMap<>(); //instantiates blank map named firstAndLast

		for (String strings : words){
			firstAndLast.put(strings.substring(0, 1), strings.substring(strings.length() -1)); //stores first and last character in firstAndLast map
		}
		return firstAndLast; //returns filled list (filled through for each loop)
	}
	...
	public Map<String, Integer> last2Revisited(String[] words) {
		//need to return a map, so instantiate a new map
		Map<String, Integer> endCounts = new HashMap<>();
		for(String  tempName : words){
			//keep track of last two characters in string
			String end = tempName.substring(tempName.length() - 2);
				//need to compare end string with portions of current string
//				tempName.substring(0, 2).equals(end);
//				tempName.substring(1, 3).equals(end);
				//tempName.length() -2 because you want the loop to not analyze last two characters of string
			int endCount = 0;
			for(int i = 0; i < tempName.length() - 2; i++)	{
				if(tempName.substring(i, i+2).equals(end)){
					endCount++;
				}
			}
			//now add tempName + endCount to map
			endCounts.put(tempName, endCount);
		}
		return endCounts;
	}
