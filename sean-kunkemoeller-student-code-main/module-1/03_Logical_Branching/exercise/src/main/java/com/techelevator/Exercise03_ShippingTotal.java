...
public class Exercise03_ShippingTotal {
    ...
    public double calculateShippingTotal(int weightPounds) {
        if (weightPounds <= MAX_WEIGHT_POUNDS) {
            ...
            int weightDifference = weightPounds - MAX_WEIGHT_POUNDS;
            weightPounds -= weightDifference;
            return (UP_TO_40_LB_RATE * weightPounds) + (OVER_40_LB_RATE * weightDifference);
        }
    ...
    public double calculateShippingTotal(int weightPounds, boolean hasDiscount) {
        ...
        if (weightPounds <= MAX_WEIGHT_POUNDS) {
            shippingTotal = UP_TO_40_LB_RATE * weightPounds;
        }
        else {
            int weightDifference = weightPounds - MAX_WEIGHT_POUNDS;
            weightPounds -= weightDifference;
            shippingTotal = (UP_TO_40_LB_RATE * weightPounds) + (OVER_40_LB_RATE * weightDifference);
        }
        if(hasDiscount) {
            double priceAfterDiscount = shippingTotal * .10;
            shippingTotal = shippingTotal - priceAfterDiscount;
        }
